...
class TCMP_Manager {
    ...
    private function matchSnippet($postId, $postType, $categoriesIds, $tagsIds, $prefix, $snippet) {
        ...
        if($postId>0) {
            if(!$include && $snippet[$prefix.'CategoriesActive'] && $tcmp->Utils->inAllArray($categoriesIds, $snippet[$prefix.'Categories'])) {
                $tcmp->Log->debug('MATCH=%s SNIPPET=%s[%s] DUE TO POST=%s CATEGORIES [%s] IN [%s]'
                    , $prefix, $snippet['id'], $snippet['name'], $postId, $categoriesIds, $snippet[$prefix.'Categories']);
                $include=TRUE;
            }
            if(!$include && $snippet[$prefix.'TagsActive'] && $tcmp->Utils->inAllArray($tagsIds, $snippet[$prefix.'Tags'])) {
                $tcmp->Log->debug('MATCH=%s SNIPPET=%s[%s] DUE TO POST=%s TAGS [%s] IN [%s]'
	        ...
	        foreach ($snippets as $v) {
	            ...
	            if(!$match && $postId>0 && $v['includeLastPostsActive'] && $v['includeLastPosts']>0) {
	                ...
	                if(in_array($postId, $ids)) {
	                    $tcmp->Log->debug('INCLUDED SNIPPET=%s[%s] DUE TO IN LAST POSTS=%s', $postId, $ids);
	                    $match=TRUE;
	                }
	            }
