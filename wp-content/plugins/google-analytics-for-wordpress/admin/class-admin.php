...
class Yoast_GA_Admin extends Yoast_GA_Options {
	...
	private function show_admin_warning() {
		return ( current_user_can( 'edit_pages' ) && ( ! isset( $_GET['page'] ) || ( isset( $_GET['page'] ) && $_GET['page'] !== 'yst_ga_settings' ) ) );
	}
	...
	private function show_admin_dashboard_warning() {
		return ( current_user_can( 'edit_pages' ) && isset( $_GET['page'] ) && $_GET['page'] === 'yst_ga_dashboard' );
	}
	...
	private function google_analytics_listener() {
		...
		if ( $google_auth_code && current_user_can( 'edit_pages' ) && wp_verify_nonce( filter_input( INPUT_POST, 'yoast_ga_nonce' ), 'save_settings' ) ) {
			self::analytics_api_clean_up();
