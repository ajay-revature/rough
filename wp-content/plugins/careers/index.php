...
function email_api()
{
	add_submenu_page( 'edit.php?post_type=jobs', 'Import Job Template', 'Import Job Template', 'edit_pages', 'import-job', 'import_job_template');
}
...
?>
		...
		<div><h2>List of current uploaded file.</h2></div>
		<ul>
		<?php 
...
if($arrpop=='php' && !file_exists($file_exit))
...
{
 echo "<b><h2><font style='color:red'>Status: File is not uploaded.<br><br>Reason: 1.(Extension should be .php)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; or <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;2. Field should not be Empty</font></h2></b>";	
}
}
}
...
function mobile_and_other_image_jobs() {
	...
	// Define the meta box for displaying job post
	...
	// Define the meta box for Job Description
	...
	foreach ( $post_types as $post_type ) {
        add_meta_box('jobs_seo_meta_info', __('Search Engine Listing(SEO) for Job Listing [For Mass Import Only]', 'textdomain'),'jobs_seo_meta_info',$post_type,'normal','high');
...
function jobs_meta_info($post) {
	...
	$jobs_google = get_post_meta($post->ID, "jobs_google", true);
	$jobs_post_type= get_post_meta($post->ID, "jobs_post_type", true);
	?>
               ECHO ""
			     <div class="form-field form-required">
                    <label for="jobs_post_type">Job Post Type</label><br/>
                    <input type="text" id="jobs_post_type" name="jobs_post_type" value="<?php echo $jobs_post_type; ?>" />
                 </div>
                ...
                <br/>
				<div class="form-field form-required">
                    <label for="jobs_google">Google Structured Data Script</label>
                    <br/>
                    <textarea id="jobs_google" rows="3" cols="50" name="jobs_google"><?php echo $jobs_google ?></textarea>
                </div>
                <br/>	
                <div class="form-field form-required">
...
function jobs_save_custom_meta_data() {
	...
	update_post_meta($post->ID, "jobs_google", $_POST["jobs_google"]); 
	update_post_meta($post->ID, "jobs_post_type", $_POST["jobs_post_type"]); 
...
function jobs_single_post($postid){
	...
	$jobs_post = get_post( $postid );
        ...
        $jobs_google = get_post_meta($post->ID, "jobs_google", true)? get_post_meta($post->ID, "jobs_google", true ): "";
         ...
         <div class="row no-margin">
              <div class="col-lg-7 col-md-7 col-sm-12 blog-bottom-border">
                    ...
                    <?php if(($jobs_post->post_content)!="")
				{
					?>
				  <div class="blog-post-single">
				 <div class="career-paragraph">
				 
				  
			         <?php echo apply_filters('the_content', $jobs_post->post_content); ?>
					 </div>
					 

					   <p style="font-size:5px;color:#fff;"><?php echo $jobs_crawled_text; ?></p>
					 </div>
...
<?php echo $jobs_google; ?>

        <?php
    }
function jobs_single_post_with_experience($postid){
        global $post;
	$jobs_post = get_post( $postid );
		...
		$jobs_post_type = get_post_meta($post->ID, "jobs_post_type", true)? get_post_meta($post->ID, "jobs_post_type", true ): "";
         ...
         <div class="row no-margin">
              <div class="col-lg-7 col-md-7 col-sm-12 blog-bottom-border">
				...
				?>
					 ...
					 <div class="blog-post-single">
				<div class="career-paragraph">
			        <?php  include(plugin_dir_path( __FILE__ ) . '/uploads/job_description_template_experience.php');?>
					</div>
					 <p style="font-size:5px;color:#fff;"><?php echo $jobs_crawled_text; ?></p>
                ...
                <div class="row form-label-txt">
                    <form id="requestForm" method="post" target="confirmFrame" action="https://qa2.revature.com/contactus" enctype="multipart/form-data">
						...
						<label class="control-label col-xs-12 text-left" for="Phone1">Phone*</label>
							...
							<div class="col-lg-12 col-xs-12 form-group inline-no-padding">
							<label class="control-label col-xs-12" for="GraduationMonth">Coding Expeience*</label>
							<div class="col-xs-12">
												<select class="shortfield-home form-control col-xs-6" id="codingexperience" name="GraduationMonth" required>
												<option value=""></option>
												...
												<span class="help-block" id="codingexperience" style="display:none;">Please select coding experience</span>
							</div>
...
<script type="text/javascript">
  var jQueryloc = jQuery.noConflict();
jQueryloc( document ).ready(function() {
var code = {};
jQueryloc("select[name='Location'] > option,select[name='Department'] > option").each(function () {
	
	
    if(code[this.text]) {
        jQueryloc(this).remove();
    } else {
        code[this.text] = this.value;
            ...
            <form id="" method="get" target="" action="<?php echo 'http://'.$_SERVER['SERVER_NAME'].'/rough/career-page';?>">
