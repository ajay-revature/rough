...
if( $avatar_left ) {
	echo '<span class="mtphr-dnt-twitter-avatar" style="width:'.$_mtphr_dnt_twitter_avatar_dimensions.'px;">'.$avatar_image.'</span>';
	...
	switch( $key ) {
		...
		case 'avatar':
			if( $display == 'on' && !$avatar_left ) {
				mtphr_dnt_twitter_get_template_part( 'assets/avatar' );
			}
			break;

		case 'name':
			if( $display == 'on' ) {
				mtphr_dnt_twitter_get_template_part( 'assets/name' );
			}
			break;

		case 'text':
			if( $display == 'on' ) {
				mtphr_dnt_twitter_get_template_part( 'assets/text' );	
			}
			break;
			
		case 'image':
			if( $display == 'on' && mtphr_dnt_twitter_has_image($mtphr_dnt_twitter_item) ) {
				mtphr_dnt_twitter_get_template_part( 'assets/image' );
			}
			break;
			
		case 'video':
			if( $display == 'on' && mtphr_dnt_twitter_has_video($mtphr_dnt_twitter_item) ) {
				mtphr_dnt_twitter_get_template_part( 'assets/video' );
			}
			break;

		case 'time':
			if( $display == 'on' ) {
				mtphr_dnt_twitter_get_template_part( 'assets/time' );
			}
			break;

		case 'links':
			if( $display == 'on' && !$direct_link ) {
				mtphr_dnt_twitter_get_template_part( 'assets/links' );
			}
			break;
	}
}
