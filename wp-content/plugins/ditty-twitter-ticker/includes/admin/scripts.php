...
function mtphr_dnt_twitter_admin_scripts( $hook ) {
	...
	if( $hook == 'ditty_news_ticker_page_mtphr_dnt_settings' || $typenow == 'ditty_news_ticker' ) {
		...
		wp_register_style( 'ditty-twitter-ticker', plugins_url('ditty-twitter-ticker/assets/css/style-admin.css'), false, MTPHR_DNT_TWITTER_VERSION );
		wp_enqueue_style( 'ditty-twitter-ticker' );
		wp_register_script( 'ditty-twitter-ticker', plugins_url('ditty-twitter-ticker/assets/js/admin/script.js'), array('jquery'), MTPHR_DNT_TWITTER_VERSION, true );
