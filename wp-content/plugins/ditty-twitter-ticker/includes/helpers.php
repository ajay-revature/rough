...
/* --------------------------------------------------------- */
/* !Get a users home timeline - 2.1.3 */
/* --------------------------------------------------------- */
...
function mtphr_dnt_twitter_oauth( $url, $args, $fields=false, $token_secret=true ) {

	$settings = mtphr_dnt_twitter_settings();
	$hash_key = $settings['secret'].'&';
	$hash_key .= $token_secret ? $settings['token_secret'] : '';
...
/* --------------------------------------------------------- */
/* !Trim tweet text of media links - 2.1.3 */
/* --------------------------------------------------------- */
...
/* --------------------------------------------------------- */
/* !Update transients - 2.1.3 */
/* --------------------------------------------------------- */

if( !function_exists('mtphr_dnt_twitter_update_transients') ) {
function mtphr_dnt_twitter_update_transients( $name, $value='', $settings=false, $delete=false ) {
	
	$settings =  $settings ? $settings : mtphr_dnt_twitter_settings();
	$cachetime = isset($settings['cache_time']) ? intval($settings['cache_time']) : 10;
	
	// Update the save transients
	if( $delete ) {
		delete_transient( $name );
		if( isset($setting['transients'][$name]) ) {
			unset($setting['transients'][$name]);
		}
	} else {
		set_transient( $name, $value, $cachetime*MINUTE_IN_SECONDS );
		$settings['transients'][$name] = $name;	
	}
...
/* --------------------------------------------------------- */
/* !Delete all transients - 2.1.3 */
/* --------------------------------------------------------- */

if( !function_exists('mtphr_dnt_twitter_delete_transients') ) {
function mtphr_dnt_twitter_delete_transients( $settings=false ) {
	$settings =  $settings ? $settings : mtphr_dnt_twitter_settings();
	
	if( is_array($settings['transients']) && count($settings['transients']) > 0 ) {
		foreach( $settings['transients'] as $i=>$transient ) {
