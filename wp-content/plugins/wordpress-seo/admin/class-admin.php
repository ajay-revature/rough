...
class WPSEO_Admin {
	...
	public function import_plugin_hooks() {
		if ( current_user_can( $this->get_edit_pages_cap() ) ) {
	...
	function register_settings_page() {
		...
		$edit_pages_cap = $this->get_edit_pages_cap();
		...
		$admin_page = add_menu_page( 'Yoast SEO: ' . __( 'Dashboard', 'wordpress-seo' ), __( 'SEO', 'wordpress-seo' ) . ' ' . $counter, $edit_pages_cap, self::PAGE_IDENTIFIER, array(
			$this,
			'load_page',
		), $icon_svg, '99.31337' );
		...
		global $submenu;
		if ( isset( $submenu[ self::PAGE_IDENTIFIER ] ) && current_user_can( $edit_pages_cap ) ) {
	...
	/**
	 * Returns the edit_pages cap
	 *
	 * @return mixed|void
	 */
	private function get_edit_pages_cap() {
		/**
		 * Filter: 'wpseo_edit_pages_capability' - Allow changing the capability users need to view the settings pages
		 *
		 * @api string unsigned The capability
		 */
		$edit_pages_cap = apply_filters( 'wpseo_edit_pages_capability', 'edit_pages' );

		return $edit_pages_cap;
	}
