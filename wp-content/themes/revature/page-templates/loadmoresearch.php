...
) AND wp_posts.post_type = 'jobs' AND ((wp_posts.post_status = 'publish')) GROUP BY wp_posts.ID ORDER BY wp_posts.post_title DESC LIMIT ".$_POST["start"].", ".$_POST["limit"]."";
...
{
	   $args = "SELECT SQL_CALC_FOUND_ROWS  wp_posts.ID, wp_posts.post_title,wp_posts.post_content FROM wp_posts  INNER JOIN wp_postmeta ON ( wp_posts.ID = wp_postmeta.post_id ) WHERE 1=1  AND ( 
  ( wp_postmeta.meta_key = 'meta_checkbox' AND wp_postmeta.meta_value = 'yes' )
) AND wp_posts.post_type = 'jobs' AND ((wp_posts.post_status = 'publish')) GROUP BY wp_posts.ID ORDER BY wp_posts.post_date DESC LIMIT ".$_POST["start"].", ".$_POST["limit"]."";
}
	     global $wpdb;
...
<?php  foreach($jobs_posts as $post){
 ...
 echo $jobs_post_type_to_upper=strtoupper($jobs_post_type);
 if($jobs_post_type_to_upper=='EXPERIENCED')
 {
					   $dir = plugins_url()."/careers/uploads/job_description_template_experience.php";
					   $handle = fopen($dir, 'r');
                        echo $data = fread($handle,163).".."."<br>";
				   ...
				   {
					  $dir = plugins_url()."/careers/uploads/job_description_template.php";
					  $handle = fopen($dir, 'r');
                       echo $data = fread($handle,163).".."."<br>";
