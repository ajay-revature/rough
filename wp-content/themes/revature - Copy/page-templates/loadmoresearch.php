...
{
 $args = "SELECT SQL_CALC_FOUND_ROWS  re_posts.ID, re_posts.post_title FROM re_posts  INNER JOIN re_postmeta ON ( re_posts.ID = re_postmeta.post_id )  INNER JOIN re_postmeta AS mt1 ON ( re_posts.ID = mt1.post_id )  INNER JOIN re_postmeta AS mt2 ON ( re_posts.ID = mt2.post_id ) WHERE 1=1  AND ( 
  ( re_postmeta.meta_key = 'meta_checkbox' AND re_postmeta.meta_value = 'yes' ) 
  AND 
  ( 
...
) AND re_posts.post_type = 'jobs' AND ((re_posts.post_status = 'publish')) GROUP BY re_posts.ID ORDER BY re_posts.post_title ASC LIMIT ".$_POST["start"].", ".$_POST["limit"]."";
 
}
else if($_POST["limit"]!="" && $_POST["start"]!="" && $_POST["s1"]!="")
{

$args = "SELECT SQL_CALC_FOUND_ROWS  re_posts.ID, re_posts.post_title FROM re_posts  INNER JOIN re_postmeta ON ( re_posts.ID = re_postmeta.post_id )  INNER JOIN re_postmeta AS mt1 ON ( re_posts.ID = mt1.post_id ) WHERE 1=1  AND ( 
  ( re_postmeta.meta_key = 'jobs_work_location' AND re_postmeta.meta_value = '".$_POST["s1"]."' ) 
  AND 
  ( 
...
{
$args = "SELECT SQL_CALC_FOUND_ROWS  re_posts.ID , re_posts.post_title FROM re_posts  INNER JOIN re_postmeta ON ( re_posts.ID = re_postmeta.post_id )  INNER JOIN re_postmeta AS mt1 ON ( re_posts.ID = mt1.post_id ) WHERE 1=1  AND ( 
  ( re_postmeta.meta_key = 'jobs_department' AND re_postmeta.meta_value = '".$_POST["sS"]."' ) 
  AND 
  ( 
    ( mt1.meta_key = 'meta_checkbox' AND mt1.meta_value = 'yes' )
  )
...
{
	  $args = "SELECT SQL_CALC_FOUND_ROWS  re_posts.ID, re_posts.post_title FROM re_posts  INNER JOIN re_postmeta ON ( re_posts.ID = re_postmeta.post_id ) WHERE 1=1  AND ( 
  ( re_postmeta.meta_key = 'meta_checkbox' AND re_postmeta.meta_value = 'yes' )
) AND re_posts.post_type = 'jobs' AND ((re_posts.post_status = 'publish')) GROUP BY re_posts.ID ORDER BY re_posts.post_date ASC LIMIT ".$_POST["start"].", ".$_POST["limit"]."";
}
	     global $wpdb;
